...
export class PlayerController {
  ...
  constructor(scene: THREE.Scene, world: RapierPhysicsWorld, camera: THREE.PerspectiveCamera, input: InputManager) {
    ...
    // Flashlight
    this.flashlight = new THREE.SpotLight(0xffffff, 10, 100, Math.PI / 4, 0.5, 1)
  ...
  createAstronautPlaceholder() {
    ...
    const visorMat = new THREE.MeshStandardMaterial({ color: 0x333333, metalness: 1, roughness: 0.1 })
  ...
  private checkGrounded(upVec: THREE.Vector3) {
    ...
    const ray = new RAPIER.Ray(rayStart, { x: rayDir.x, y: rayDir.y, z: rayDir.z })
    
    const hit = this.physicsWorld.world.castRay(ray, 1.2, true, undefined, undefined, this.body.collider(0))
  ...
  private handleMovement(dt: number, upVec: THREE.Vector3) {
    ...
    const forward = new THREE.Vector3().crossVectors(upVec, new THREE.Vector3().crossVectors(camDir, upVec)).normalize()
    ...
    if (moveDir.lengthSq() > 0) {
      ...
      const force = moveDir.multiplyScalar(currentSpeed * (this.isGrounded ? 50.0 : 10.0))
      ...
      if (speed > this.maxVelocity) {
        ...
        this.body.setLinvel({ x: vel.x * ratio, y: vel.y * ratio, z: vel.z * ratio }, true)
      }
    ...
    if (this.input.jump && this.isGrounded) {
      ...
      this.body.applyImpulse({ x: jumpImpulse.x, y: jumpImpulse.y, z: jumpImpulse.z }, true)
  ...
  private updateInVehicle(dt: number) {
    ...
    const upVec = this.physicsWorld.applySphericalGravity(this.currentVehicle.body, dt)
    this.orbitCamera.targetDistance = 15
  ...
  dismount() {
    ...
    const dismountPos = this.currentVehicle.mesh.position.clone().add(upVec.multiplyScalar(3))

    this.body.setEnabled(true)
    this.body.setTranslation({ x: dismountPos.x, y: dismountPos.y, z: dismountPos.z }, true)
