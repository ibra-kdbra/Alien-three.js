...
export class Vehicle {
  ...
  update(dt: number, input?: InputManager) {
    ...
    this.suspensionPoints.forEach((p) => {
      // Offset origin UPWARDS to ensure it's not starting inside the ground
      const worldPoint = p
        .clone()
      ...
      const rayOrigin = worldPoint
        .clone()
        .add(upVec.clone().multiplyScalar(2.0));

      // Raycast down from offset point
      const ray = new RAPIER.Ray(
        { x: rayOrigin.x, y: rayOrigin.y, z: rayOrigin.z },
        { x: down.x, y: down.y, z: down.z },
      );

      // Max distance is originOffset + hoverHeight
      ...
      if (hit) {
        const distance = ((hit as any).toi ?? (hit as any).time) - 2.0; // Subtract origin offset
        const offset = this.hoverHeight - distance;

        if (offset > 0) {
          // Spring Force
          const velAtPoint = this.body.velocityAtPoint({
            x: worldPoint.x,
            y: worldPoint.y,
            z: worldPoint.z,
          });
          const velProj = new THREE.Vector3(
            velAtPoint.x,
            velAtPoint.y,
            velAtPoint.z,
          ).dot(upVec);
    ...
    if (alignment < 0.99) {
      ...
      this.body.applyTorqueImpulse(
        { x: axis.x * 20, y: axis.y * 20, z: axis.z * 20 },
        true,
      );
    }
